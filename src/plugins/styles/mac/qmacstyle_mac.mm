...
Q_LOGGING_CATEGORY(lcMacStyle, "qt.widgets.styles.macos");
...
void QMacStylePrivate::drawProgressBar(QPainter* p, const QStyleOptionProgressBar *pb) const
{
    ...
    if (@available(macOS 14.0, *)) { // silence compiler
        ...
        // Draw track / progress
        p->setPen(Qt::NoPen);
        p->setBrush(pb->state & QStyle::State_Active
            ? qt_mac_toQBrush([NSColor controlAccentColor]).color() : Qt::lightGray);
        p->drawRoundedRect(track, radius, radius);
        p->restore();
    }
#else
    // This function should only be used when qt_apple_runningWithLiquidGlass() returns true.
    // It will not do that for SDKs < macosx14.0.
    Q_UNUSED(p);
    Q_UNREACHABLE();
#endif
}
